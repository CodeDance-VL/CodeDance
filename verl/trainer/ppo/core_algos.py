...
#     n_step = (torch.diff(response_mask, dim=1) == 1).sum(dim=1, keepdim=True)
#     if n_step[0] > 1:
#         breakpoint()
#     negative_approx_kl = log_prob - old_log_prob
...
def compute_policy_loss_gspo(
    ...
    # compute step-level importance ratio per step (runs of 1s) in a vectorized way (no padding)
    mask_i = response_mask.to(dtype=torch.long)
    mask_f = response_mask.to(dtype=log_prob.dtype)
    B, T = mask_i.shape

    # start of a step occurs at t==0 with mask==1, or when diff==1 (0->1)
    starts = torch.zeros_like(mask_i)
    starts[:, 0] = mask_i[:, 0]
    diff01 = mask_i[:, 1:] - mask_i[:, :-1]
    starts[:, 1:] = (diff01 == 1).to(mask_i.dtype)

    num_steps = starts.sum(dim=1)
    max_steps = int(num_steps.max().item())

    if max_steps <= 1:
        step_importance_log = torch.zeros_like(response_mask, dtype=log_prob.dtype)
    else:
        #breakpoint()
        # step id per token: cumsum(starts) - 1; valid only where mask==1
        step_ids = torch.cumsum(starts, dim=1) - 1
        step_ids = step_ids.clamp(min=0).to(torch.long)

        # aggregate sums and counts per step via scatter_add
        step_sums = torch.zeros(B, max_steps, device=log_prob.device, dtype=log_prob.dtype)
        step_sums.scatter_add_(1, step_ids, (negative_approx_kl * mask_f))
        step_lens = torch.zeros(B, max_steps, device=log_prob.device, dtype=log_prob.dtype)
        step_lens.scatter_add_(1, step_ids, mask_f)

        step_avg = step_sums / step_lens.clamp(min=1.0)

        # broadcast back to tokens
        step_importance_log = torch.gather(step_avg, dim=1, index=step_ids)
        step_importance_log = step_importance_log * mask_f

    # Combine at token level with stop-grad on step importance
    log_step_importance_ratio = log_prob - log_prob.detach() + step_importance_log.detach()
    log_step_importance_ratio = torch.clamp(log_step_importance_ratio, max=10.0)

    # finally exp() to remove log
    seq_importance_ratio = torch.exp(log_step_importance_ratio)
