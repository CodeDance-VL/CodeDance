...
class SGLangRollout(BaseRollout):
    ...
    async def _handle_engine_call_batch(
        self,
        reqs: List[AsyncRolloutRequest],
        do_sample: bool,
        is_validate: bool,
        **kwargs,
    ) -> List[Dict]:
        ...
        generation_prompt_ids_list = [
            req.get_generation_prompt(self.tokenizer, self.processor) for req in reqs
        ]
        
        # Use a common max_new_tokens for the batch
        max_new_tokens = min(
            min(self.config.response_length, self.config.max_model_len - len(ids) - 1)
            for ids in generation_prompt_ids_list
        )
        ...
        # Prepare batched image data
        image_list_batch = [req.get_image_list() for req in reqs]
        
        with self.update_sampling_params(**kwargs):
            outputs = await self._engine.async_generate(
                input_ids=generation_prompt_ids_list,
                sampling_params=self.sampling_params,
                return_logprob=False,
                image_data=image_list_batch,
            )
        return outputs

    async def _turn_based_req_level_generate_sequences(
        self,
        ...
        **kwargs,
        ...
        active_reqs = list(req_list)
        completed_reqs = []
        
        turn_count = {req.request_id: 0 for req in req_list}
        
        while active_reqs:
            # 1. Handle state transitions for requests that are not waiting for generation.
            # PENDING -> RUNNING (includes tool initialization)
            pending_reqs = [req for req in active_reqs if req.state == AsyncRolloutRequestStateEnum.PENDING]
            if pending_reqs:
                tool_init_tasks = []
                for req in pending_reqs:
                    if req.tools:
                        for tool_schema in req.tools:
                            tool = self._tool_map[tool_schema.function.name]
                            create_kwargs = req.tools_kwargs[tool.name].get("create_kwargs", {})
                            tool_init_tasks.append(tool.create(req.request_id, **create_kwargs))
                if tool_init_tasks:
                    await asyncio.gather(*tool_init_tasks)
                for req in pending_reqs:
                    req.state = AsyncRolloutRequestStateEnum.RUNNING

            # TOOL_CALLING -> RUNNING or COMPLETED (includes tool execution)
            tool_calling_reqs = [req for req in active_reqs if req.state == AsyncRolloutRequestStateEnum.TOOL_CALLING]
            if tool_calling_reqs:
                tool_exec_tasks = {} # req_id -> list of tasks
                for req in tool_calling_reqs:
                    if req.messages[-1].tool_calls:
                        parsed_tool_calls = req.messages[-1].tool_calls
                        tasks = [
                            self._tool_map[tool_call.function.name].execute(
                                req.request_id,
                                tool_call.function.arguments,
                                **req.tools_kwargs[tool_call.function.name].get("execute_kwargs", {}),
                            )
                            for tool_call in parsed_tool_calls
                        ]
                        tool_exec_tasks[req.request_id] = asyncio.gather(*tasks)

                if tool_exec_tasks:
                    all_results = await asyncio.gather(*tool_exec_tasks.values())
                    results_map = dict(zip(tool_exec_tasks.keys(), all_results))
                    
                    for req in tool_calling_reqs:
                        if req.request_id in results_map:
                            tool_call_results = results_map[req.request_id]
                            parsed_tool_calls = req.messages[-1].tool_calls
                            
                            finish_reason_type = None
                            for i, (tool_call, (resp, reward, metrics)) in enumerate(zip(parsed_tool_calls, tool_call_results)):
                                add_state = req.add_tool_response_message(self.tokenizer, resp, (i == len(parsed_tool_calls) - 1), format=self.config.multi_turn.format, processor=self.processor)
                                if not add_state:
                                    finish_reason_type = FinishReasonTypeEnum.STOP
                                    break
                            
                            if len(req.input_ids) >= self.config.max_model_len or finish_reason_type == FinishReasonTypeEnum.STOP:
            ...
            # 2. Collect all requests that are now ready for generation.
            running_reqs = [req for req in active_reqs if req.state == AsyncRolloutRequestStateEnum.RUNNING]

            # 3. Batch generate for running_reqs
            if running_reqs:
                reqs_to_run = []
                for req in running_reqs:
                    if turn_count[req.request_id] >= self.config.multi_turn.max_turns:
                        req.state = AsyncRolloutRequestStateEnum.COMPLETED
                    else:
                        reqs_to_run.append(req)

                if reqs_to_run:
                    outputs = await self._handle_engine_call_batch(reqs_to_run, do_sample, is_validate, **kwargs)
                    
                    for req, output in zip(reqs_to_run, outputs):
                        turn_count[req.request_id] += 1
                        content = output["text"]
                        finish_reason = FinishReasonTypeEnum.from_str(output["meta_info"]["finish_reason"]["type"])
                        
                        if finish_reason == FinishReasonTypeEnum.LENGTH:
                            req.add_assistant_message(self.tokenizer, content, already_over_long=True, format=self.config.multi_turn.format)
                            req.state = AsyncRolloutRequestStateEnum.COMPLETED
                        else:
                            if self._function_call_parser and self._function_call_parser.has_tool_call(content):
                                ...
                                for tool_call in tool_calls:
                                    ...
                                    if not has_decode_error:
                                        parsed_tool_calls.append(OpenAIFunctionToolCall(id=str(tool_call.tool_index), function=function))

                                if parsed_tool_calls:
                                    req.add_assistant_message(self.tokenizer, normed_content, tool_calls=parsed_tool_calls, format=self.config.multi_turn.format)
                                    req.state = AsyncRolloutRequestStateEnum.TOOL_CALLING
                                else:
                                    req.add_assistant_message(self.tokenizer, content, format=self.config.multi_turn.format)
                                    req.state = AsyncRolloutRequestStateEnum.COMPLETED
                            else:
                                req.add_assistant_message(self.tokenizer, content, format=self.config.multi_turn.format)
                                req.state = AsyncRolloutRequestStateEnum.COMPLETED
            ...
            # 4. Update active_reqs list for next iteration and move completed to a separate list
            next_active_reqs = []
            for req in active_reqs:
                if req.state == AsyncRolloutRequestStateEnum.COMPLETED:
                    completed_reqs.append(req)
                elif req.state != AsyncRolloutRequestStateEnum.FAILED:
                    next_active_reqs.append(req)
            active_reqs = next_active_reqs
        
        # 5. Finalization
        finalization_tasks = []
        async def finalize_req(req):
            ...
            req.finalize(self.tokenizer, tool_reward_scores, FinishReasonTypeEnum.STOP, processor=self.processor)

        for req in completed_reqs:
            finalization_tasks.append(finalize_req(req))
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self._tp_rank == 0:
            ...
            output_req_list = loop.run_until_complete(
                self._turn_based_req_level_generate_sequences(
                    req_list, do_sample, is_validate, **kwargs
                )
            )
        ...
        if len(prompt_position_ids[0].size()) == 1:
            ...
            prompt_position_ids = pad_position_ids_3d(prompt_position_ids, max_length=self.config.prompt_length, batch_first=True, padding_value=0)
            delta_position_id = delta_position_id.unsqueeze(1).repeat(1, prompt_position_ids.shape[1], 1)
    ...
    async def chat_completion(self, json_request):
        ...
        output = await self._handle_engine_call_batch([req], True, False, **json_request)
    ...
    def _process_multimodal_data(self, req, tgt_device):
        ...
        images = req.get_image_list() if hasattr(req, 'get_image_list') else []
        device_multi_modal_inputs = {}
        ...
        multi_modal_inputs = {k: v for k, v in multi_modal_data.items()    if k not in ["input_ids", "attention_mask"]}
        return device_multi_modal_inputs, {"image": images}
...
def pad_position_ids_3d(position_ids_list, batch_first=True, padding_value=0, padding_side="left", max_length=1024):
    ...
    if dim0_padded.shape[-1] < max_length:
        dim0_padded = pad_sequence_to_length(dim0_padded, max_length, 0, left_pad=True)
        dim1_padded = pad_sequence_to_length(dim1_padded, max_length, 0, left_pad=True)
        dim2_padded = pad_sequence_to_length(dim2_padded, max_length, 0, left_pad=True)
    if batch_first:
        ...
        if isinstance(msg_dict.get("content"), list):
            ...
            for item in msg_dict["content"]:
                if item.get("type") == "text":
                    ...
                    content_items.append(ImageContent(type="image", image=item.get("image", multi_modal_data['image'][img_ids])))
                    img_ids += 1
            msg = Message(
                role=msg_dict["role"],
