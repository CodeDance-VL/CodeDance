...
class SGLangRollout(BaseRollout):
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        while current_turns < self.config.multi_turn.max_assistant_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if len(parsed_tool_calls) > 0:  
                    try:
                        TOOL_EXECUTION_TIMEOUT=125.0
                        tool_call_results = await asyncio.wait_for(
                            asyncio.gather(
                                *[
                        ...
                        logger.error(f"Tool execution for request {_req.request_id} timed out after {TOOL_EXECUTION_TIMEOUT}s.")
                        error_response = [{"content":[{"type":"text", "text":"Error: Tool execution timed out and was cancelled."}]}]
                        tool_call_results = [ (resp, 0.0, {}) for resp in error_response ] 

                    except Exception as e:
                        # 捕获其他可能的工具执行错误
                        logger.error(f"An unexpected error occurred during tool execution for request {_req.request_id}: {e}", exc_info=True)
                        error_response = [{"content":[{"type":"text", "text":f"Error: An unexpected error occurred in tool: {e}"}]}]
                        tool_call_results = [ (resp, 0.0, {}) for resp in error_response ]
                    #breakpoint()
                    if _req.state == AsyncRolloutRequestStateEnum.COMPLETED:
                        _req.add_tool_response_messages(self.tokenizer, [resp for resp, _, _ in tool_call_results], processor=self.processor)
                        break

                    if current_turns == self.config.multi_turn.max_assistant_turns - 1:
                        tool_call_results.append(({"content":[{"type":"text", "text":"Turn limit reached. Based on the analysis so far, directly provide the final answer using the <think></think><answer></answer> format. Do not use any more tools or execute code."}]}, None, None))
                    state = _req.add_tool_response_messages(self.tokenizer, [resp for resp, _, _ in tool_call_results], processor=self.processor)
                    # if len(_req.input_ids) >= self.config.max_model_len:
                    #     finish_reason_type = FinishReasonTypeEnum.STOP
                    #     break
                    if not state: # TODO: exceed max model length
                        finish_reason_type = FinishReasonTypeEnum.STOP
                ...
                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    _req.add_assistant_message(self.tokenizer, content, processor=self.processor)
                    break
                else:
        ...
        #     if msg.role == "tool" and msg.get_image_content():
        #         has_image = True
        #         break
        # if has_image:
        #     breakpoint()
        #     chatml_messages = [msg.to_chatml_format() for msg in _req.messages]
        #     raw_prompt = self.tokenizer.apply_chat_template(  # type: ignore
        #         conversation=chatml_messages,
        #         tools=[tool.model_dump() for tool in _req.tools] if _req.tools else None,
        #         add_generation_prompt=False,
        #         tokenize=False,
        #     )
        #     multi_modal_data = self.processor(
        #     text=[raw_prompt],
        #     images=_req.get_image_list(download_urls=True),
        #     videos=None,
        #     return_tensors="pt"
        #     )
        #     print("here")
        #     if(len(multi_modal_data['input_ids'][0]) != len(_req.input_ids)):
        #         print(f"gen prompt len: {self.tokenizer.decode(multi_modal_data['input_ids'][0])}  self len: {self.tokenizer.decode(_req.input_ids)}")
        ...
        _req.finalize(self.tokenizer, tool_reward_scores, finish_reason_type, processor=self.processor)
        return _req
    
    async def _handle_engine_call(
        self, _req: AsyncRolloutRequest, sampling_params: dict) -> dict:
        generation_prompt_ids = _req.get_generation_prompt(self.tokenizer, self.processor)
        kwargs = sampling_params.copy()
        max_new_tokens = min(self.config.response_length, self.config.max_model_len - len(generation_prompt_ids) - 1)

        effective_max_model_len = min(self.config.max_model_len, self.config.prompt_length + self.config.response_length)
        if len(generation_prompt_ids) > effective_max_model_len:
            generation_prompt_ids = generation_prompt_ids[-effective_max_model_len:]

        remaining_space = effective_max_model_len - len(generation_prompt_ids) - 1
        max_new_tokens = max(0, min(self.config.response_length, remaining_space))

        kwargs["max_new_tokens"] = max_new_tokens
        kwargs["n"] = 1
        
        # prepare image data
        image_list = _req.get_image_list()
        output = await self._engine.async_generate(
            input_ids=generation_prompt_ids,
            sampling_params=kwargs,
            return_logprob=False,
            image_data=image_list if len(image_list) > 0 else [],  # pass image data
        )
        return output

    def _extract_python_code_from_response(self, response_text: str) -> Optional[str]:
        """Extract Python code from ```python code blocks."""
        # 匹配 ```python 开始到 ``` 结束的代码块
        pattern = r'```python\s*\n(.*?)\n```'
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self._tp_rank == 0:
            ...
            # Split req_list into micro-batches to avoid OOM
            micro_batch_size = self.config.multi_turn.get("micro_batch_size", 32)
            output_req_list = []
        ...
        reward_scores = []
        num_turns = []
        multi_modal_inputs_list = []  # Store multimodal inputs for each request
        multi_modal_data_list = [] # suport multimodal data
        for req in sorted_output_req_list:
            assert req.state == AsyncRolloutRequestStateEnum.COMPLETED, f"Request {req.request_id} is not completed"
            assert len(req.input_ids) == len(req.attention_mask) == len(req.position_ids if isinstance(req.position_ids[0], int) else req.position_ids[0]) == len(req.loss_mask), f"""Request {req.request_id} has different length of 
            ...
            # Multimodal supportances
            multi_modal_inputs, multi_modal_data = self._process_multimodal_data(req, tgt_device)
            multi_modal_inputs_list.append(multi_modal_inputs)
            multi_modal_data_list.append(multi_modal_data)
            # Serialize messages with multimodal content preservation. TODO: No multi-modal reserved
            # chatml_messages = [msg.to_chatml_format() for msg in req.messages]
            # messages.append({"messages:":chatml_messages})
            # reward_scores.append(req.reward_scores)
            num_turns.append(getattr(req, "num_turns", 0))
        ...
        #breakpoint()
        # The logic for creating delta_position_id is incorrect here, because response_position_ids
        # are already calculated incrementally in the AsyncRolloutRequest.
        # We just need to pad them to the same length.
        if len(prompt_position_ids) > 0 and len(prompt_position_ids[0].size()) == 1:
            ...
            response_position_ids = pad_sequence(response_position_ids, batch_first=True, padding_value=0, padding_side="right")
            if response_position_ids.shape[-1] < self.config.response_length:
                response_position_ids = pad_sequence_to_length(response_position_ids, self.config.response_length, 0, left_pad=False)

        else:
            prompt_position_ids = pad_position_ids_3d(prompt_position_ids, max_length=self.config.prompt_length, batch_first=True, padding_value=0, padding_side="left")
            response_position_ids = pad_position_ids_3d(response_position_ids, max_length=self.config.response_length, batch_first=True, padding_value=0, padding_side='right')
        ...
        # Restart code execution workers after the batch, only on the global rank 0 process.
        # if self._tp_rank == 0:
        #     if self._tool_map: 
        #         loop = asyncio.get_event_loop()
        #         for tool in self._tool_map.values():
        #             if hasattr(tool, "restart_all_workers"):
        #                 logger.info("Restarting all code execution workers at the end of the batch.")
        #                 if loop.is_running():
        #                     future = asyncio.run_coroutine_threadsafe(tool.restart_all_workers(), loop)
        #                     future.result()  # Wait for the coroutine to finish
        #                 else:
        #                     loop.run_until_complete(tool.restart_all_workers())
        ...
        non_tensor_batch = {}
        # free cache engine has been moved into the micro-batch loop
        if self._engine is not None and self._tp_rank == 0:
            loop = asyncio.get_event_loop()
            loop.run_until_complete(self._engine.flush_cache())
        non_tensor_batch["num_turns"] = np.array(num_turns)

        if len(multi_modal_data_list) != 0:
        ...
        for data_idx, raw_prompt in enumerate(prompts.non_tensor_batch["raw_prompt"]):
            for rollout_offset in range(n):
                ...
                if "multi_modal_data" in prompts.non_tensor_batch:
                    multi_modal_data = prompts.non_tensor_batch["multi_modal_data"][data_idx]
                    if len(multi_modal_data["image"]) == 0:
                        print("no image in multi_modal_data")
                else:
                    print("no multi_modal_data")
                    multi_modal_data = None
                
                #if "multi_modal_inputs" in prompts.non_tensor_batch:
                #    multi_modal_inputs = prompts.non_tensor_batch["multi_modal_inputs"][data_idx]
                messages = raw_prompt_to_msg(raw_prompt, multi_modal_data)
                if self._tool_schemas:
                    _tools_kwargs = prompts.non_tensor_batch["tools_kwargs"][data_idx]
                    _tool_schemas = []
                    for k in _tools_kwargs.keys():
                        _tool_schemas.append(self._tool_map[k].get_openai_tool_schema())
                        # Convert messages to ChatML format for processor
                        chatml_messages = [msg.to_chatml_format() for msg in messages]
                        raw_prompt_text = self.tokenizer.apply_chat_template(
                            conversation=chatml_messages,
                            add_generation_prompt=False,
                            tokenize=False,
                        )                    
                    # Handle multimodal processing with tools
                    if self.processor is not None and multi_modal_data:
                        ...
                        _input_ids = model_inputs["input_ids"][0].tolist()
                        _attention_mask = model_inputs["attention_mask"][0].tolist()
                        
                        # Handle special position IDs for Qwen2VL
                        if self.processor.image_processor.__class__.__name__ == "Qwen2VLImageProcessor":
                            ...
                            _position_ids = compute_position_id_with_mask(model_inputs["attention_mask"][0]).tolist()
                        
                        # Store additional multimodal inputs
                        ...
                        # Text-only processing with tools
                        input_data = self.tokenizer(raw_prompt_text, return_tensors="pt", add_special_tokens=False)
                    ...
                    if len(_input_ids) > self.config.prompt_length:
                        ...
                        if truncate_right:
                            ...
                            _position_ids = _position_ids[: self.config.prompt_length] if isinstance(_position_ids[0], int) else [_pos_ids[: self.config.prompt_length] for _pos_ids in _position_ids]
                        else:
                            _input_ids = _input_ids[-self.config.prompt_length:]
                            _attention_mask = _attention_mask[-self.config.prompt_length:]
                            _position_ids = _position_ids[-self.config.prompt_length:] if isinstance(_position_ids[0], int) else [_pos_ids[-self.config.prompt_length:] for _pos_ids in _position_ids]
                    ...
                    # No tools case - handle multimodal data if present
                    if self.processor is not None and multi_modal_data:
                ...
                req = AsyncRolloutRequest(
                    ...
                    messages=messages,  # Use converted Message objects with multimodal support
                ...
                position_len = len(_position_ids) if isinstance(_position_ids[0], int) else len(_position_ids[0])
                error_message = f"Request {req.request_id} has mismatched lengths: input_ids={len(req.input_ids)}, attention_mask={len(req.attention_mask)}, position_ids={position_len}, loss_mask={len(req.loss_mask)}"

                assert len(req.input_ids) == len(req.attention_mask) == position_len == len(req.loss_mask), error_message
    ...
    def _process_multimodal_data(self, req, tgt_device):
        """Process multimodal data for a single request."""
        images = req.get_image_list() 
        raw_prompt = self.tokenizer.apply_chat_template(  # type: ignore
            conversation=[msg.to_chatml_format() for msg in req.messages],
            #tools=[tool.model_dump() for tool in req.tool_schemas] if req.tool_schemas else None,
            add_generation_prompt=False,
            tokenize=False,
        )
        # HuggingFace processors expect batched inputs. Use 1-element lists.
        if len(images) == 0:
            # Pure text request – skip multimodal inputs to prevent empty
            # `image_grid_thw` tensors that break the visual encoder.
            print("no image")
            return {}, {"image": []}
        # if len(images) > 1:
        #     breakpoint()
        ...
        # Remove text-related keys; they are already present in the main batch
        multi_modal_data.pop("input_ids", None)
        multi_modal_data.pop("attention_mask", None)
        return dict(multi_modal_data), {"image": images}
        
def pad_position_ids_3d(position_ids_list, batch_first=True, padding_value=0, padding_side="left", max_length=1024):
    if not position_ids_list or len(position_ids_list[0].size()) == 1:
        return pad_sequence(position_ids_list, batch_first=batch_first, 
                          padding_value=padding_value, padding_side=padding_side)
    
    dim0_list = [pos_ids[0, :] for pos_ids in position_ids_list]
    dim1_list = [pos_ids[1, :] for pos_ids in position_ids_list]  
    dim2_list = [pos_ids[2, :] for pos_ids in position_ids_list]
    
    dim0_padded = pad_sequence(dim0_list, batch_first=batch_first, 
                              padding_value=padding_value, padding_side=padding_side)
    dim1_padded = pad_sequence(dim1_list, batch_first=batch_first,
                              padding_value=padding_value, padding_side=padding_side)
    dim2_padded = pad_sequence(dim2_list, batch_first=batch_first,
                              padding_value=padding_value, padding_side=padding_side)
    assert dim0_padded.shape == dim1_padded.shape == dim2_padded.shape
    if dim0_padded.shape[-1] < max_length:
        left_pad = padding_side == "left"
        dim0_padded = pad_sequence_to_length(dim0_padded, max_length, 0, left_pad=left_pad)
        dim1_padded = pad_sequence_to_length(dim1_padded, max_length, 0, left_pad=left_pad)
        dim2_padded = pad_sequence_to_length(dim2_padded, max_length, 0, left_pad=left_pad)
    if batch_first:
        return torch.stack([dim0_padded, dim1_padded, dim2_padded], dim=1)
    else:
        return torch.stack([dim0_padded, dim1_padded, dim2_padded], dim=2)
