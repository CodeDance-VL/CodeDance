...
class SGLangRollout(BaseRollout):
            ...
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    for i, (tool_call, (resp, reward, metrics)) in enumerate(zip(parsed_tool_calls, tool_call_results)):
                        # TODO: single tools now
                        add_state = _req.add_tool_response_message(self.tokenizer, resp, (i == len(parsed_tool_calls) - 1), format=self.config.multi_turn.format, processor=self.processor)
                        if not add_state:
    ...
    async def _handle_engine_call(self, _req: AsyncRolloutRequest, do_sample: bool, is_validate: bool, **kwargs) -> dict:
        ...
        with self.update_sampling_params(**kwargs):
            output = await self._engine.async_generate(
                ...
                image_data=image_list if len(image_list) > 0 else None,  # pass image data
            )
        return output
    ...
    def _process_multimodal_data(self, req, tgt_device):
        ...
        #             device_multi_modal_inputs[key] = value.to(tgt_device)
        #         else:
        #             device_multi_modal_inputs[key] = value
